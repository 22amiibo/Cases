...
test("Exhibit Analysis completes Observe, Prioritize, Interpret, Act", async ({ page }) => {
  ...
  await page.getByLabel("Your reasoning").fill("Labor rose much more than every other category.");
  ...
  await page.getByLabel("Your reasoning").fill("Labor is the clearest candidate, but the cause needs testing.");
...
test("Hypothesis updates a claim through two evidence rounds", async ({ page }) => {
  ...
  await page.getByLabel("Rationale").fill("Price or mix may have weakened despite volume growth.");
  ...
  await page.getByLabel("Rationale").fill("Faster cost growth weakens the revenue explanation.");
  await page.getByRole("button", { name: "Commit update" }).click();
  await page.getByRole("checkbox", { name: "Use this evidence in the update" }).check();
  await page.getByLabel("Rationale").fill("Concentrated overtime supports the labor claim.");
  await page.getByRole("button", { name: "Commit update" }).click();
