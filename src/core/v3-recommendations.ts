...
export function selectV3Recommendation({ activities, attempts, skills, runs = [], courseStep = null }: {
  ...
  const candidates = skills.flatMap(s => s.diagnostics).flatMap(diagnostic => {
  ...
  for (const skill of [...skills].filter(s => s.status === "Developing" && s.transferred === 0).sort((a, b) => (b.latestCompletedAt ?? "").localeCompare(a.latestCompletedAt ?? "") || a.skillId.localeCompare(b.skillId))) {
    ...
    if (activity) return recommendation(activity, "transfer", `${skill.reviewed} reviewed ${skill.label.toLowerCase()} practice${skill.reviewed === 1 ? "" : "s"}; no retry or case application yet. Apply the skill in a new context.`);
  }
...
export function findRecoverableRuns(entries: [string, string][], resources: ProgressResource[]): RecoverableRun[] {
